    ...
    <Card className="mt-4">
      ...
      <div className="flex flex-col gap-4">
        ...
        {brandSamples.length > 0 && (
          <div>
            <div className="text-[12px] font-medium text-[var(--text-2)] mb-1.5">브랜드 제품 샘플 제안</div>
            <div className="flex flex-col gap-1.5">
              {brandSamples.map((s) => (
                <Link key={s.id} href={`/launches/${s.project?.id || ''}`}>
                  <div className="flex flex-wrap items-center gap-2 px-2.5 py-2 rounded-md border border-[var(--border-1)] bg-[var(--bg-1)] hover:bg-[var(--bg-2)] transition-colors">
                    <Badge tone="neutral" size="xs">
                      샘플
                    </Badge>
                    <span className="text-[12.5px] text-[var(--text-1)] flex-1 min-w-[140px] truncate">
                      {s.project?.productName || '(삭제된 제품)'}
                    </span>
                    {s.quantity ? (
                      <span className="text-[11px] text-[var(--text-4)]">{s.quantity}</span>
                    ) : null}
                    <span className="text-[11px] text-[var(--text-3)] tabular">
                      납기 {fmtDate(s.dueDate)}
                    </span>
                  </div>
                </Link>
              ))}
